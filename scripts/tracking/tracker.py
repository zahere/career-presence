...
class ApplicationTracker:
    ...
    def add_application(self, app: Application) -> bool:
        ...
        with self._get_connection() as conn:
            try:
                ...
                """, (
                    app.id, app.company, app.role, app.job_url, app.platform,
                    app.status, app.match_score,
                    app.resume_variant, app.cover_letter,
                    app.salary_range, app.location, app.remote_policy,
                    app.applied_at, app.response_at,
                    json.dumps(app.keywords_matched), app.notes,
    ...
    def get_analytics(self) -> Dict[str, Any]:
        ...
        with self._get_connection() as conn:
            ...
            # Response rate
            applied_count = stats.get("applied", 0) + stats.get("responded", 0) + \
                           stats.get("interviewing", 0) + stats.get("offer", 0) + \
                           stats.get("rejected", 0)
            responded_count = stats.get("responded", 0) + stats.get("interviewing", 0) + \
                             stats.get("offer", 0) + stats.get("rejected", 0)
            ...
            return {
                ...
                "avg_score_by_status": {row["status"]: round(row["avg_score"], 1) for row in avg_scores if row["avg_score"]}
            }
...
def main():
    ...
    tracker.add_interaction(
        "app_12345",
        "note",
        "Strong alignment with AgentiCraft work"
    )
